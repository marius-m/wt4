...
class JiraFilterWorklog(
    ...
    override fun valid(input: WorkLog?): Boolean {
        ...
        if (input.author == null || (input.author.email == null) || (input.author.name == null)) throw JiraFilter.FilterErrorException("Worklog is invalid")
        if ((input.author.email != user) && (input.author.name != user)) throw JiraFilter.FilterErrorException("Worklog does not belong to the user")
